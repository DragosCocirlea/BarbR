package ro.akaaka.barbr.activities.SignIn
...
import ro.akaaka.barbr.activities.Main.MainActivity
import ro.akaaka.barbr.activities.Register.RegisterActivity
...
class SignInActivity : AppCompatActivity() {
    ...
    private fun checkTokens() {
        val prefs = this.getSharedPreferences("ro.akaaka.barbr.prefs", 0)
    ...
    private fun tokensStillValid(responseData: String) {
        ...
        val prefsEdit = this.getSharedPreferences("ro.akaaka.barbr.prefs", 0).edit()
    ...
    private fun successfulSignIn(alreadySignedIn : Boolean, access: String = "", refresh: String = "") {
        if (!alreadySignedIn) {
            ...
            reqUserData.response { _ , response, _ ->
                when {
                    response.statusCode >= 200 -> {
                        ...
                        val prefsEdit = applicationContext.getSharedPreferences("ro.akaaka.barbr.prefs", 0).edit()
                        prefsEdit.putString("user_email", userEmail)
                        prefsEdit.putString("user_name", userName)
                        prefsEdit.putString("user_phone", userPhone)
                        prefsEdit.putString("user_birthday", userBirthday)
            ...
            val prefsEditor = this.getSharedPreferences("ro.akaaka.barbr.prefs", 0).edit()
            prefsEditor.putString("access_token", access)
            prefsEditor.putString("refresh_token", refresh)
            prefsEditor.apply()
        }

        val prefs = getSharedPreferences("ro.akaaka.barbr.prefs", 0)
